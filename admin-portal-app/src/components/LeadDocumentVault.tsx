...
export default function LeadDocumentVault({
  ...
  const handleFiles = async (files: FileList | File[]) => {
    ...
    if (newDocs.length > 0) {
      const currentList = lead.documents || [];
  ...
  const handleDownloadDocument = (doc: LeadDocument) => {
    ...
    link.href = previewBlobUrl || doc.dataUrl;
  ...
  const handleOpenInNewWindow = (doc: LeadDocument) => {
    const urlToOpen = previewBlobUrl || doc.dataUrl;
    if (urlToOpen) {
      const win = window.open();
      if (win) {
        win.document.write(`
          <html>
            <head><title>${doc.name} - Enrol Overseas Viewer</title></head>
            <body style="margin:0;background:#0f172a;display:flex;align-items:center;justify-content:center;height:100vh;">
              ${isImageDoc(doc) ? `<img src="${urlToOpen}" style="max-width:100%;max-height:100%;object-fit:contain;" />` : 
                isPdfDoc(doc) ? `<iframe src="${urlToOpen}" style="width:100%;height:100%;border:none;"></iframe>` :
                `<a href="${urlToOpen}" download="${doc.name}" style="color:white;font-family:sans-serif;font-weight:bold;text-decoration:none;padding:12px 24px;background:#0284c7;border-radius:12px;">Download ${doc.name}</a>`}
            </body>
          </html>
        `);
      }
