...
// Helper to validate if a key is a valid Supabase JWT key (starts with 'ey' and has 3 segments separated by dots)
  ...
  if (supabase) {
    try {
      const { data, error } = await supabase
        ...
        .select('*');
      if (!error && data) {
        leadsToSync = data.map(unpackLeadWithMetadata);
      }
    } catch (e) {
